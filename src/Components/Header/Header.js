import { BellDot, HamIcon, Menu, Search, User, Video } from 'lucide-react'
import React from 'react'

const Header = () => {
  return (
    <> 
    <div className='w-full border py-4 flex justify-between'>
        <div>

          


          <div className='w-16 h-10 flex '>
          <div className='pl-4'>
            <Menu/>
          </div>

            <img className='px-2' src="data:image/png;base64,  " alt=""/>
            <h1 className='text-2xl font-bold'>Youtube</h1>
          </div>
          

        </div>
        <div className='flex '> 
            <input className='border py-2 rounded-lg' type="text"  placeholder='search' name="" value=""/>
            <Search className='mt-2'/>
          </div>
          <div className='flex'>
            <p className='px-2'><Video/></p>
            <p className='px-2'><BellDot/></p>
            <p className='px-2 pr-4'><User/></p>
          </div>
    </div>
    </>
  )
}

export default Header